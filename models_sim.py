    ...
    def forward_sim(self, x1, x2, boxes1, rel_pos_21, mm, update_mm, mask=None):
        ...
        with torch.no_grad():
            ...
            pred_boxes_features = self.extract_box_feature(x=pred, boxes_info=boxes1, scale_factor=1. / self.patch_size,
                                                           mask=mask)
            target_boxes_features = self.extract_box_feature(x=target, boxes_info=boxes1.detach(), scale_factor=1. / self.patch_size,
                                                             mask=mask)
...
def sim_vit_base_patch16_dec512d8b(**kwargs):
    model = SiameseIMViT(
        patch_size=16, img_size=352, embed_dim=768, depth=12, num_heads=12,
