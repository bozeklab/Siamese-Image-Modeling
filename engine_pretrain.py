...
def train_one_epoch(model: torch.nn.Module,
    ...
    for data_iter_step, data in enumerate(metric_logger.log_every(data_loader, print_freq, header)):
        if args.pred_shape in ['attmask_high', 'attmask_hint', 'attmask_low'] and epoch >= args.pred_start_epoch:
        ...
        if args.loss_type in ['sim',]:
            ...
            with torch.cuda.amp.autocast(enabled=(not args.fp32)):
                loss, outputs, attn = model(x1, x2, rel_pos_21, mm, update_mm, attn_mask=attn_mask,
                                            adios_mask=args.adios_masking,
                                            spatial_x1=spatial_x1,
                                            mask=mask)
            ...
            #vattn = torchvision.utils.make_grid(vattn.permute(0, 3, 1, 2))
            del attn
        else:
